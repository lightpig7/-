...
void SetKey(char My_key[8],bool SubKey[16][48]); //生成16轮的子密钥；
...
void Run_Des(char My_message[8], char HexMssage[16],bool SubKey[16][48]);//des的轮加密算法
...
void Run_desDes(char My_message[8], char HexMessage[16],bool SubKey[16][48]);// DES轮解密算法；

std::string runDES_encode(char *My_message,char *My_key)
{
    std::string result;
    ...
    SetKey(My_key,SubKey);  //生成16轮的加密子密钥；
    Run_Des(My_message, Message_hex,SubKey); //des的轮加密过程
    //printf("经过加密的密文为:\n");
    for(i = 0; i < 16; i++)
    {
        char tmp[100];
        sprintf(tmp, "%c", Message_hex[i]);
...
std::string runDES_decode(char *Message_hex,char *You_key){
    std::string result;
    static bool SubKey[16][48] = {0};
    int i = 0, j;
    char My_message[8] = {0};
    ...
    SetKey(You_key,SubKey);  //生成16轮的解密子密钥；

    Run_desDes(My_message, Message_hex,SubKey);//解密;
    //printf("解密结果为:\n");

    for(i = 0; i < 8; i++)
    {
        char tmp[100];
        sprintf(tmp, "%c", My_message[i]);
...
void SetKey(char My_key[8],bool SubKey[16][48])
{
    int i, j;
...
void Run_Des(char My_message[8], char HexMssage[16],bool SubKey[16][48])//des轮加密算法；
{
    int i;
