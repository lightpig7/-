...
void runAES(uint8_t in[16], uint8_t key[32], uint8_t out[16])
{
...
void runAES_de(uint8_t msg[16],uint8_t key[32], uint8_t out[16])
{
    ...
    // 解密
    inv_cipher(out, msg, w);

    printf("msg:\n");
    for (i = 0; i < 4; i++) {
        printf("%x %x %x %x ", msg[4*i+0], msg[4*i+1], msg[4*i+2], msg[4*i+3]);
    }
